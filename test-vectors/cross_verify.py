...
# Verify-only runners: these files are not regenerated by this script but are
# cross-verified against an independent implementation. Run after GENERATORS.
VERIFIERS = {
    "signing_hybrid": "signing_hybrid_cross_verify.py",
}
...
def run(target: Optional[str] = None) -> None:
    ...
    # Run verify-only cross-verifiers (not generators — they verify pre-built vectors).
    verifier_targets = {target: VERIFIERS[target]} if target and target in VERIFIERS else (
        VERIFIERS if not target else {}
    )
    for name, script in verifier_targets.items():
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                failed.append((script, result.stdout + result.stderr))
                print(f"  ✗ {script} — {result.stdout.strip() or result.stderr.strip()}")
        except OSError as e:
