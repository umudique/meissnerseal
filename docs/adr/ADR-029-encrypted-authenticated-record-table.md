...
1. **One primitive, both properties.** The item record table is sealed with AEAD
   (XChaCha20-Poly1305, key = MEK, AAD = vault_id ‖ schema_profile ‖ table version
   counter). This yields integrity **and** metadata confidentiality together —
   consistent with crypto_design.md §9 "no unauthenticated encryption anywhere."
...
- vault_format_v1.md §5–§7 are revised: encrypted+authenticated table, fixed WRK
  position, table version counter in AEAD AAD, optional bucketed padding, new
  `schema_profile`. This revision must land **before CORE-8** (multi-record
  persist) so the format is built once.
