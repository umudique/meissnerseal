...
const SEALED_TABLE_SECTION_OVERHEAD: usize = 4 + 24 + meissnerseal_crypto::aead::TAG_LEN;
...
fn fresh_id() -> Result<[u8; 16]> {
    meissnerseal_crypto::rng::random_bytes(16)
        .try_into()
...
fn build_item_frame(
    ...
    let mut rek_bytes: [u8; 32] = meissnerseal_crypto::rng::random_bytes(32)
        .try_into()
        .map_err(|_| CoreError::Crypto)?;
...
///
/// ## Invariants
/// - Uses only `meissnerseal-crypto` for RNG and AEAD operations; this crate never
