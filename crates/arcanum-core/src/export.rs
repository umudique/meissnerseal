...
///
/// ## Invariants
/// - arcanum-core provides no plaintext export path; unsafe plaintext import or
///   export flags belong only in arcanum-cli.
/// - Uses only `arcanum-crypto` for Argon2id, RNG, and AEAD operations; this
...
///
/// ## Invariants
/// - All cryptography is delegated to `arcanum-crypto`; arcanum-core does not
...
mod tests {
    ...
    fn unique_temp_vault_path(label: &str) -> std::path::PathBuf {
        ...
        let nanos = std::time::SystemTime::now()
            ...
            .unwrap_or(0);
        path.push(format!(
            "arcanum-core-export-{label}-{}-{nanos}.arcv",
            std::process::id()
        ));
        path
    }
