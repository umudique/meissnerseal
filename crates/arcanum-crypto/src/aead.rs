...
/// - `nonce` is a 24-byte `XChaCha20Nonce` generated by `generate_nonce` in
///   production code; caller-supplied nonces are limited to tests and vector
///   verification.
...
/// ## Postconditions
/// - On success, returns `Ciphertext` containing `ciphertext_bytes || tag`.
/// - On success, returned length is `plaintext.len() + TAG_LEN`.
...
mod tests {
    ...
    fn test_encrypt_produces_expected_ciphertext_and_tag() {
        ...
        let ciphertext = encrypt(&key, &nonce, &PLAINTEXT, &AAD).expect("encrypt vector");
    ...
    fn test_empty_plaintext_ok() {
        let key = AeadKey::from_bytes(KEY_BYTES);
        let nonce = XChaCha20Nonce::from_bytes(NONCE_BYTES);
        let ciphertext = encrypt(&key, &nonce, &[], &AAD).expect("encrypt empty plaintext");
